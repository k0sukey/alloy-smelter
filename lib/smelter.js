...
program.version(require('../package.json').version)
	...
	.option('-f, --force', 'force coping or moving even if file already exists')
...
program.command('i18n <add,remove> <lang>')
	.description(chalk.gray('Internationalization dir/file management'))
	.action(function(subcommand, lang){
		require('../lib/i18n')({
...
program.command('stats')
	.description(chalk.gray('Statistics a Alloy project'))
	.action(function(){
		require('../lib/stats')({
				program: program
			});
...
program.command('const')
	.description(chalk.gray('Edit the constants options'))
	.action(function(){
		var dir = path.join(process.env.HOME, '.smelter');
		fs.ensureDir(dir, function(err){
			if (err) {
				...
				editor(path.join(dir, 'constants.json'));
			}
		});
	});
