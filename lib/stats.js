...
module.exports = function(_params){
	...
	_.each(views, function(_view){
		...
		_.each(_tags, function(_tag){
			...
			if (_attributes = _tag.match(/(\w+)=("[^<>"]*"|'[^<>']*'|\w+)/g)) {
				_.each(_attributes, function(_attribute){
					var _tmp = _attribute.split('='),
						_name = _tmp[0],
						_value = _tmp[1].replace(/['"]/ig, '');

					if (_name === 'id' || _name === 'class') {
						return;
					}

					if (_name.match(/^on.+/ig) &&
						_.indexOf(CONST.SPECIAL_PROPERTY_NAMES, _name) === -1) {
						return;
					}

					if (_.indexOf(CONST.BIND_PROPERTIES, _name) > -1) {
						if (_.has(bindings, _name)) {
							bindings[_name]++;
						} else {
							bindings[_name] = 1;
						}
					...
					if (_.has(properties, _name)) {
						properties[_name]++;
					} else {
						properties[_name] = 1;
					}

					if (_name.match(/color/i)) {
						var _color = _value.toLowerCase();
