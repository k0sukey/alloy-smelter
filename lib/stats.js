...
module.exports = function(_params){
			...
			if (_events = _tag.match(/\son.+?=['"].+?['"]/ig)) {
				_.each(_events, function(_event){
					var _tmp = _event.split('='),
						_type = _tmp[0].replace('on', '').toLowerCase(),
	...
	for (var key in events.type) {
		sortable.push([key, events.type[key]]);
	}
	...
	events.type = [];
	_.each(sortable, function(_item){
		events.type.push(sprintf('%30s: ', _item[0]) + sprintf('%3s', _item[1]));
	});

	sortable = [];

	for (var key in events.listener) {
		sortable.push([key, events.listener[key]]);
	}
	...
	events.listener = [];
	_.each(sortable, function(_item){
		events.listener.push(sprintf('%30s: ', _item[0]) + sprintf('%3s', _item[1]));
	});
	...
	table.push(['Event type', events.type.join('\n')]);
	table.push(['Event listener', events.listener.join('\n')]);
